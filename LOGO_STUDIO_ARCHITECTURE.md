...
- **Phase 4 — Typography + shapes + SVG.** **Real work starts here.** Add `flutter_svg` (render) + a hand-rolled minimal SVG path editor (parse `<path d=...>` into editable point/curve handles — no mature Flutter package does two-way SVG-path editing, this will be original code per Section 3's "implement natively" fallback). Consolidate the two font lists into one `TypographyScale` + a shared font-picker widget. Sanitize any imported SVG (strip `<script>`, external refs, event handlers) before rendering.
