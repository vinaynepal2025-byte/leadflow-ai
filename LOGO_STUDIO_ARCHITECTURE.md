...
- **Phase 5 — Templates + asset library.** ✅ Asset library shipped: `tenant_assets` table + `/tenant-assets` CRUD, toolbar picker (filter by kind, upload, save-current-SVG, long-press remove). **Deferred:** the data-driven Template Engine (turning `logoTemplates.js`'s 7 hardcoded templates into DB rows + letting users save their own custom templates) — today's `GenerateLogoScreen` template flow is real and functional, just not user-extensible. Tracked as a follow-up increment, same status as Phase 4's path-node editing.
...
- **Phase 7 — Export pipeline.** ✅ Real multi-format raster export shipped: `POST /flyer-projects/:id/export` converts the client's captured PNG to JPG/WebP via `sharp`, and to a genuine favicon `.ico` via a hand-rolled PNG-in-ICO container (verified: valid ICO header, embedded PNG intact — see commit). Archives every export to the Asset Library (kind='export') as a non-fatal background step. Size presets already existed pre-session (`kFlyerCanvasPresets`, `_exportAllSizes()`) and needed no rework. **Deferred:** true vector SVG export of the whole canvas — the canvas isn't itself an SVG document (mixed Flutter widgets/CustomPainter/flutter_svg/text), so a correct whole-canvas-to-SVG serializer is a substantial standalone effort; shipping a partial/approximate one would risk exported SVGs visibly not matching what the user designed, which the master spec's "No Fake Features" section rules out. Tracked as a follow-up.
- **Phase 8 — AI generation + variants.** ✅ Shipped: `POST /flyer-projects/:id/ai-generate-logo` reuses `aiProvider.js`'s `generateJson()` to compose a real 2-4 element icon+shape+text logo mark (not the flyer prompt's promotional-layout style, and not a pick from the 7 fixed templates) — validated/clamped exactly like the existing flyer `ai-generate`, output is genuinely editable `canvas_json`. This is the actual generative-AI gap the audit flagged, closed. Logo Studio's "Generate with AI" now branches on `isLogoMode` to call this instead of the flyer endpoint. Variants (icon-only, text-only, mono black/white, dark/light background) are mechanical transforms over the current elements as planned — each becomes its own duplicated, independently-editable project via the Phase 6 duplicate endpoint, no new "variant group" concept needed.
