...
**Recommended fix:** before Phase 1 schema work begins, generate one `SCHEMA_SNAPSHOT.sql` by introspecting the live Postgres `information_schema` (read-only, safe), commit it, and require all new tables from this point forward to go through `backend/migrations/`.
...
- No connection-level tenant isolation (e.g., Postgres RLS) — isolation is 100% application-code discipline (`WHERE tenant_id = ?` on every query). One missed clause in one query is a cross-tenant data leak. Worth auditing systematically before Organization→Workspace tenancy adds a second isolation dimension.
