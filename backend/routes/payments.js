...
router.post('/initiate', async (req, res) => {
  ...
  const fee = await db.prepare('SELECT * FROM fee_payments WHERE tenant_id = ? AND id = ?').get(tid, fee_payment_id);
  ...
  try {
    ...
    if (gateway === 'khalti') {
      const lead = await db.prepare('SELECT full_name, phone FROM leads WHERE id = ?').get(fee.lead_id);
...
async function markPaid(tid, link) {
  await db.prepare("UPDATE payment_links SET status = 'paid', paid_at = datetime('now') WHERE id = ?").run(link.id);
  await db.prepare("UPDATE fee_payments SET status = 'paid', paid_date = date('now'), payment_method = ? WHERE id = ?")
    .run(link.gateway, link.fee_payment_id);

  const fee = await db.prepare('SELECT * FROM fee_payments WHERE id = ?').get(link.fee_payment_id);
  const lead = await db.prepare('SELECT full_name FROM leads WHERE id = ?').get(fee.lead_id);
...
router.post('/razorpay/verify', async (req, res) => {
  ...
  const link = await db.prepare('SELECT * FROM payment_links WHERE tenant_id = ? AND id = ?').get(tid, link_id);
  ...
  try {
    ...
    if (!valid) {
      await db.prepare("UPDATE payment_links SET status = 'failed' WHERE id = ?").run(link.id);
      return res.status(400).json({ error: 'Signature verification failed — payment not confirmed' });
    }
    await markPaid(tid, link);
...
router.get('/khalti/verify/:linkId', async (req, res) => {
  const tid = tenantId(req);
  const link = await db.prepare('SELECT * FROM payment_links WHERE tenant_id = ? AND id = ?').get(tid, req.params.linkId);
  if (!link) return res.status(404).json({ error: 'Payment link not found' });
  ...
  try {
    ...
    await db.prepare('UPDATE payment_links SET status = ? WHERE id = ?').run(result.status === 'Expired' ? 'expired' : 'created', link.id);
