...
router.post('/', async (req, res) => {
  ...
  res.status(201).json(await db.prepare('SELECT * FROM reminders WHERE id = ?').get(id));
});

// PATCH /reminders/:id — mark done / missed / reschedule
router.patch('/:id', async (req, res) => {
  const tid = tenantId(req);
  const existing = await db.prepare('SELECT * FROM reminders WHERE tenant_id = ? AND id = ?').get(tid, req.params.id);
  ...
  await db.prepare(`UPDATE reminders SET ${updates.join(', ')} WHERE tenant_id = ? AND id = ?`).run(...values);
  res.json(await db.prepare('SELECT * FROM reminders WHERE id = ?').get(req.params.id));
});

// DELETE /reminders/:id
router.delete('/:id', async (req, res) => {
  const tid = tenantId(req);
  const result = await db.prepare('DELETE FROM reminders WHERE tenant_id = ? AND id = ?').run(tid, req.params.id);
