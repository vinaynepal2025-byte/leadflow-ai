...
router.patch('/:id', async (req, res) => {
  const tid = tenantId(req);
  const existing = await db.prepare('SELECT * FROM tasks WHERE tenant_id = ? AND id = ?').get(tid, req.params.id);
  ...
  await db.prepare(`UPDATE tasks SET ${updates.join(', ')} WHERE tenant_id = ? AND id = ?`).run(...values);
